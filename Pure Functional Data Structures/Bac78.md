...
적용형 컴퓨터 시스템의 이력 민감성과 저장공간의 부재는 그들이 컴퓨터 디자인의 기틀을 제공하지 못한 주요 원인입니다. 더욱이, 가장 적용적인 시스템들은 람다 대수의 부분 연산들을 그들의 기본 연산으로 채용했습니다. 이 연산들은 가장적으로 무제한적인 힘 중 하나이나, 그들의 완전하고 효율적인 현실화는 기계 설계자에게 큰 어려움을 제공합니다. 더욱이, 저장소를 가져와 폰 노이만 시스템의 효율성을 향상시키는 노력은 응용 시스템들로 하여금 큰 폰 노이만 시스템에 휩싸이게끔 하는 경향을 띕니다. 예를 들어, 순수 Lisp은 종종 폰 노이만 기능이 많은 대규모 확장에 묻혀 있습니다. 복잡한 시스템을 재구성하는 것은 기계 설계자에게 거의 지침을 제공하지 않습니다.

## 폰 노이만 프로그램과 함수형 프로그램의 비교

 폰 노이만 언어들의 문제점들을 더 자세히 보기 위해, 기존의 프로그래밍 언어와 후술할 함수형 언어로 작성한 내적 계산을 한 것을 비교할 것이다.
...
위 프로그램의 몇 몇 속성들은 가치가 없다  
a) 이 명령문은 복잡한 규칙에 따라 보이지 않는 상태에서 동작한다.  
b) 위계적이지 않다. 배정문의 우측 피연산자를 제외하고는 간단한 개체로부터 복잡한 개체로 구성되지 않는다.  
c) 동적이고 반복적이다. 정신적으로 수행해야 이해할 수 있다.  
d) 반복에 의해 한번에 한 단어를 수행한다.  
e) 데이터의 일부분인 n 이 프로그램 안에 위치한다. 이는 일반화를 해칠 수 있고, n 길이의 벡터에서만 동작한다.  
f) 자체의 인자에 이름을 부여한다. 이는 오로지 벡터 a와 b에만 적용할 수 있다. 보다 일반적이게 되기 위해서는, 절차적인 선언이 필요하다. 이는 복잡한 문제를 야기한다(이름에 의한 호출, 값에 의한 호출)
g) "하우스키핑" 작업은 흩어져 있는 기호로 표시된다(for 문 및 할당의 아래 첨자). 이로 인해 가장 일반적인 하우스키핑 작업을 강력하고 널리 유용한 단일 운영자로 통합하는 것이 불가능하다. 따라서 이러한 작업을 프로그래밍할 때는 항상 첫 번째 정사각형에서 다시 시작해야 하며 "for i .--- ..." 및 "for j := ..." 뒤에 i와 j가 뿌려진 배정문이 옵니다.
...
합성(˚), 삽입(/), 그리고 모두에 적용(α) 은 기존에 존재하는 함수들을 조합하여 새로운 형태를 합성하는 합수형 형태입니다. 따라서 f˚g는 g와 f를 순서대로 받고, αf는 인자의 모든 멤버에 f를 적용하는 것입니다.